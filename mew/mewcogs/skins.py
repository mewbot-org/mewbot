...
from mewutils.misc import get_pokemon_image, get_file_name, pagify, MenuView, ConfirmView
import asyncio
import random
import time
...
BUYABLE_SKINS = {
    "glimmer": [
        "staryu", "azurill", "hoppip", "raikou", "entei",
        "suicune", "ho-oh", "luvdisc", "buneary", "stunky",
        "finneon", "phione", "manaphy", "petilil", "skrelp",
        "dedenne", "carbink", "klefki", "yungoos",
    ],
    "sketch": [
        "bulbasaur", "charmander", "squirtle", "igglybuff", "zubat",
        "growlithe", "geodude", "gastly", "voltorb", "mewtwo",
        "mew",
    ],
    "zodiac": [
        "skiddo", "tauros", "morpeko", "krabby", "shinx",
        "audino", "baltoy", "skorupi", "rowlet", "wooloo",
        "popplio", "wishiwashi",
    ],
}
...
class Skins(commands.Cog):
    ...
    @skin.command()
    @discord.app_commands.describe(pokemon="The Pokémon number you want to apply the skin on", skin="The name of the Skin to apply.")
    @tradelock
    async def apply(self, ctx, pokemon: int, skin: str):
        """Apply a skin to a pokemon."""
        poke = pokemon
        async with ctx.bot.db[0].acquire() as pconn:
            data = await pconn.fetchrow("SELECT skins::json, pokes[$1] FROM users WHERE u_id = $2", poke, ctx.author.id)
            if data is None:
                await ctx.send("You have not started!\nStart with `/start` first.")
                return
            skins, pid = data
            data = await pconn.fetchrow("SELECT pokname, skin, shiny, radiant FROM pokes WHERE id = $1", pid)
            if data is None:
            ...
            confirm = f"Are you sure you want to apply your {skin} skin to your {pokname}?\n"
            ...
            if skin in BUYABLE_SKINS:
                await pconn.execute("UPDATE pokes SET tradable = false WHERE id = $1", pid)
    ...
    @skin.command()
    @discord.app_commands.describe(pokemon="The Pokémon number you want to preview the skin on", skin="The name of the Skin to preview.")
    async def skin_preview(self, ctx, pokemon: str, skin: str):
        ...
        if skin in BUYABLE_SKINS:
            ...
            if form_poke is None:
                ...
                return
            search_poke = await ctx.bot.db[1].pfile.find_one({"id": form_poke["base_id"]})
            while search_poke["identifier"] not in BUYABLE_SKINS[skin]:
                if search_poke["evolves_from_species_id"] == "":
                    ...
                    return
                search_poke = await ctx.bot.db[1].pfile.find_one({"id": search_poke["evolves_from_species_id"]})
    ...
    async def shop(self, ctx):
        ...
        async with ctx.bot.db[0].acquire() as pconn:
            shards = await pconn.fetchval("SELECT skin_tokens FROM users WHERE u_id = $1", ctx.author.id)
        if shards is None:
            await ctx.send("You have not started!\nStart with `/start` first.")
            return
        embed = discord.Embed(
    ...
    @skin.command()
    @discord.app_commands.describe(pokemon="The Pokémon name you want to buy the skin for", skin="The name of the Skin to buy.")
    @tradelock
    async def skin_buy(self, ctx, pokemon: str, skin: str):
        ...
        if skin not in skins:
            await ctx.send(f"You don't have the `{skin}` skin in your shop right now! View your shop with `/skin shop`.")
            return
        ...
        while search_poke["identifier"] not in skins[skin]:
            if search_poke["evolves_from_species_id"] == "":
                await ctx.send(f"You don't have a `{skin}` skin for `{poke}` in your shop right now! View your shop with `/skin shop`.")
                return
            search_poke = await ctx.bot.db[1].pfile.find_one({"id": search_poke["evolves_from_species_id"]})
        ...
        async with ctx.bot.db[0].acquire() as pconn:
            data = await pconn.fetchrow("SELECT skin_tokens, skins::json FROM users WHERE u_id = $1", ctx.author.id)
...
class RaidSpawn(discord.ui.View):
    ...
    async def start(self):
        pokeurl = "https://dyleee.github.io/mewbot-images/sprites/" + await get_file_name(self.poke, self.bot, skin=self.skin)
        ...
        form_info = await self.bot.db[1].forms.find_one({"identifier": self.poke.lower()})
        ...
        super_moves = [x["identifier"].capitalize().replace("-", " ") for x in super_raw]
        normal_raw = await self.bot.db[1].moves.find({"type_id": {"$in": normal_types}, "damage_class_id": {"$ne": 1}}).to_list(None)
        normal_moves = [x["identifier"].capitalize().replace("-", " ") for x in normal_raw]
        un_raw = await self.bot.db[1].moves.find({"type_id": {"$in": un_types}, "damage_class_id": {"$ne": 1}}).to_list(None)
        ...
        async with self.bot.db[0].acquire() as pconn:
            for attacker, damage in self.attacked.items():
                await pconn.execute("UPDATE users SET skin_tokens = skin_tokens + $1 WHERE u_id = $2", damage * 2, attacker.id)
        embed = discord.Embed(
...
class RaidMove(discord.ui.Button):
    ...
    def __init__(self, move, damage):
        ...
        self.damage = damage
        if damage == 2:
            self.effective = "It's super effective! You will get 2x rewards if the poke is defeated."
        elif damage == 1:
    ...
    async def callback(self, interaction):
        ...
        await interaction.response.send_message(content=f"You attack the alpha pokemon with {self.move}... {self.effective}", ephemeral=True)

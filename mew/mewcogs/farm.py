...
class FertilizeView(discord.ui.View):
    ...
    def __init__(
        ...
        self.add_item(
            DropdownSelectFertilize(
                berry_ids, berry_names, fertilizers, fertilizer_quantity, emojis
            )
        )
...
class DropdownSelectFertilize(discord.ui.Select):
    def __init__(
        self, berry_ids, berry_names, fertilizers, fertilizer_quantity, emojis
    ):
        self.berry_ids = berry_ids
        self.fertilizers = fertilizers
        self.fertilizer_quantity = fertilizer_quantity
        ...
        super().__init__(
            placeholder="Pick a berry...", min_values=1, max_values=1, options=options
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async with interaction.client.db[0].acquire() as pconn:
            ...
            await pconn.execute(
                "UPDATE berries SET fertilized = True WHERE berry_id = $1", berry_id
            )
            await pconn.execute(
        ...
        async with interaction.client.db[0].acquire() as pconn:
            ...
            if interval <= 0:
                ...
                await interaction.client.commondb.add_bag_item(
                    interaction.user.id, berry_name, amount_gained
                )
    ...
    async def plant(
        ...
        async with ctx.bot.db[0].acquire() as pconn:
            landplots = await pconn.fetchval(
                "SELECT landplots FROM users WHERE u_id = $1", ctx.author.id
            )
            bag = await pconn.fetchrow(
                "SELECT * FROM bag WHERE u_id = $1", ctx.author.id
            )
            active_task_count = await pconn.fetchval(
                "SELECT COUNT(u_id) FROM berries WHERE u_id = $1", ctx.author.id
            )
        ...
        if (current_seed_amount - amount) < 0:
            ...
            return
        await self.bot.commondb.remove_bag_item(ctx.author.id, seed_name, amount)
        async with ctx.bot.db[0].acquire() as pconn:
        ...
        await ctx.send(
            f"You've successfully planted {amount}x {berry} Berries!\nMake sure to water it using `/farm info`!"
        )
    ...
    async def info(self, ctx):
        ...
        async with self.bot.db[0].acquire() as pconn:
            berry_data = await pconn.fetch(
                "SELECT * FROM berries WHERE u_id = $1", ctx.author.id
            )
            player_data = await pconn.fetchrow(
                "SELECT water_tank, fertilizer FROM bag WHERE u_id = $1", ctx.author.id
            )
            landplots = await pconn.fetchval(
                "SELECT landplots FROM users WHERE u_id = $1", ctx.author.id
            )
        ...
        if len(berry_data) == 0:
            ...
            first_embed.set_footer(text="/farm plant to plant some seeds!")
            for i in range(1, landplots):
                first_embed.add_field(name=f"Plot {i}", value="Empty", inline=True)
        ...
        first_embed.set_footer(text="Use the menu below to water/pick your berries")
        length = len(berry_ids)
        for idx in range(length):
            num = idx + 1
            ...
            status = statuses[idx]
            berry_url = get_berry_emoji(name=berry_name)
    ...
    async def fertilize(self, ctx):
        ...
        async with self.bot.db[0].acquire() as pconn:
            ...
            fertilizer = await pconn.fetchval(
                "SELECT fertilizer FROM bag WHERE u_id = $1", ctx.author.id
            )
            landplots = await pconn.fetchval(
                "SELECT landplots FROM users WHERE u_id = $1", ctx.author.id
            )
        ...
        first_embed.set_footer(text="Use the dropdown menu to water/pick your berries")
        length = len(berry_ids)
        if length != 0:
            for idx in range(length):
                num = idx + 1

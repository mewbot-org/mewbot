...
class DuelPokemon:
    ...
    def __init__(self, **kwargs):
        ...
        self.starting_moves = (
            self.moves.copy()
        )  # shallow copy to keep the objects but not the list itself
        ...
        self.spdef_changes = []
        self.speed_changes = []
        self.accuracy_changes = []
        self.evasion_changes = []
    ...
    def send_out(self, otherpoke, battle):
        ...
        if (
            self.owner.toxic_spikes
            and self.grounded(battle)
            and ElementType.POISON in self.type_ids
        ):
        ...
        if self.held_item != "heavy-duty-boots":
            ...
            if self.grounded(battle):
                ...
                if self.owner.toxic_spikes == 1:
                    ...
                    msg += self.nv.apply_status(
                        "b-poison", battle, source="toxic spikes"
                    )
                # Sticky web
                if self.owner.sticky_web:
                    msg += self.append_speed(
                        StatChange(stage_delta=-1), source="the sticky web"
                    )
        ...
        if self.owner.lunar_dance:
            ...
            if self.nv.current:
                ...
                self.nv.reset()
            not_at_full_pp = [
                move for move in self.moves if move.pp != move.starting_pp
            ]
            if not_at_full_pp:
                used = True
        ...
        if self.held_item == "electric-seed" and battle.terrain.item == "electric":
            msg += self.append_defense(
                StatChange(stage_delta=1), attacker=self, source="its electric seed"
            )
            self.held_item.use()
        if self.held_item == "psychic-seed" and battle.terrain.item == "psychic":
            msg += self.append_spdef(
                StatChange(stage_delta=1), attacker=self, source="its psychic seed"
            )
            self.held_item.use()
        if self.held_item == "misty-seed" and battle.terrain.item == "misty":
            msg += self.append_spdef(
                StatChange(stage_delta=1), attacker=self, source="its misty seed"
            )
            self.held_item.use()
        if self.held_item == "grassy-seed" and battle.terrain.item == "grassy":
            msg += self.append_defense(
                StatChange(stage_delta=1), attacker=self, source="its grassy seed"
            )
            self.held_item.use()
    ...
    def send_out_ability(self, otherpoke, battle):
        ...
        # Imposter (sus)
        if (
            self.ability() == Ability.IMPOSTER
            and otherpoke is not None
            and not otherpoke.substitute
            and otherpoke.illusion_name is None
        ):
        ...
        if self.ability() == Ability.INTIMIDATE and otherpoke is not None:
            if otherpoke.ability() == Ability.OBLIVIOUS:
                ...
                msg += otherpoke.append_attack(
                    StatChange(stage_delta=-1),
                    attacker=self,
                    source=f"{self.name}'s Intimidate",
                )
                if otherpoke.held_item == "adrenaline-orb":
                    msg += otherpoke.append_speed(
                        StatChange(stage_delta=1),
                        attacker=otherpoke,
                        source="its adrenaline orb",
                    )
        ...
        if self.ability() == Ability.INTREPID_SWORD:
            msg += self.append_attack(
                StatChange(stage_delta=1), attacker=self, source="its intrepid sword"
            )
        if self.ability() == Ability.DAUNTLESS_SHIELD:
            msg += self.append_defense(
                StatChange(stage_delta=1), attacker=self, source="its dauntless shield"
            )
        if (
            self.ability() == Ability.TRACE
            and otherpoke is not None
            and otherpoke.ability_giveable()
        ):
        ...
        if self.ability() == Ability.DOWNLOAD and otherpoke is not None:
            if otherpoke.get_spdef(battle) > otherpoke.get_defense(battle):
                msg += self.append_attack(
                    StatChange(stage_delta=1), attacker=self, source="its download"
                )
            else:
                msg += self.append_spatk(
                    StatChange(stage_delta=1), attacker=self, source="its download"
                )
        ...
        if (
            self.ability() == Ability.FRISK
            and otherpoke is not None
            and otherpoke.held_item.has_item()
        ):
        ...
        if self.ability() == Ability.RKS_SYSTEM and self._name == "Silvally":
            ...
            if e is not None:
                ...
                t = ElementType(e).name.lower()
                msg += (
                    f"{self.name} transformed into a {t} type using its rks system!\n"
                )
        if self.ability() == Ability.TRUANT:
            self.truant_turn = 0
        if self.ability() == Ability.FORECAST and self._name in (
            "Castform",
            "Castform-snowy",
            "Castform-rainy",
            "Castform-sunny",
    ...
    def remove(self, battle, *, fainted=False):
        ...
        if not fainted:
            if self.ability() == Ability.NATURAL_CURE and self.nv.current:
                msg += (
                    f"{self.name}'s {self.nv.current} was cured by its natural cure!\n"
                )
                self.nv.reset()
    ...
    def next_turn(self, otherpoke, battle):
        ...
        if self.disable.next_turn():
            msg += (
                f"{self.name}'s {prev_disab_move.pretty_name} is no longer disabled!\n"
            )
        ...
        if self.held_item == "white-herb":
            changed = False
            if self.calculate_stat_stage(self.atk_changes) < 0:
                self.atk_changes = []
                changed = True
            if self.calculate_stat_stage(self.def_changes) < 0:
                self.def_changes = []
                changed = True
            if self.calculate_stat_stage(self.spatk_changes) < 0:
                self.spatk_changes = []
                changed = True
            if self.calculate_stat_stage(self.spdef_changes) < 0:
                self.spdef_changes = []
                changed = True
            if self.calculate_stat_stage(self.speed_changes) < 0:
                self.speed_changes = []
                changed = True
            if changed:
                msg += (
                    f"{self.name}'s white herb reset all negative stat stage changes.\n"
                )
                self.held_item.use()
        if self.held_item == "toxic-orb":
            msg += self.nv.apply_status(
                "b-poison", battle, attacker=self, source="its toxic orb"
            )
        if self.held_item == "flame-orb":
            msg += self.nv.apply_status(
                "burn", battle, attacker=self, source="its flame orb"
            )
        ...
        if self.held_item == "black-sludge":
            if ElementType.POISON in self.type_ids:
                ...
                msg += self.damage(
                    self.starting_hp // 8, battle, source="its black sludge"
                )

        # Abilities
        if self.ability() == Ability.SPEED_BOOST and not self.swapped_in:
            msg += self.append_speed(
                StatChange(stage_delta=1), attacker=self, source="its Speed boost"
            )
        ...
        if (
            self.ability() in (Ability.WATER_VEIL, Ability.WATER_BUBBLE)
            and self.nv.burn()
        ):
            self.nv.reset()
        ...
        if self.ability() == Ability.RAIN_DISH and battle.weather.get() in (
            "rain",
            "h-rain",
        ):
        ...
        if self.ability() == Ability.SOLAR_POWER and battle.weather.get() in (
            "sun",
            "h-sun",
        ):
        ...
        if self.ability() == Ability.MOODY:
            stats = [
                (self.atk_changes, "attack"),
                (self.def_changes, "defense"),
                (self.spatk_changes, "special attack"),
                (self.spdef_changes, "special defense"),
                (self.speed_changes, "speed"),
            ]
            add_stats = stats.copy()
            remove_stats = stats.copy()
            for stat in stats:
                if self.calculate_stat_stage(stat[0]) == 6:
                    add_stats.remove(stat)
            add_stat = None
            if add_stats:
                add_stat = random.choice(add_stats)
                msg += self.append_stat(
        ...
        if (
            self.ability() == Ability.ICE_FACE
            and not self.ice_repaired
            and self._name == "Eiscue-noice"
        ...
        if (
            self.ability() == Ability.HARVEST
            and self.last_berry is not None
            and not self.held_item.has_item()
        ):
        ...
        if (
            self.ability() == Ability.ZEN_MODE
            and self._name == "Darmanitan"
        ...
        if (
            self.ability() == Ability.ZEN_MODE
            and self._name == "Darmanitan-galar"
            and self.hp < self.starting_hp / 2
        ):
        ...
        if (
            self.ability() == Ability.ZEN_MODE
            and self._name == "Darmanitan-zen-galar"
            and self.hp >= self.starting_hp / 2
        ):
        ...
        if (
            self.ability() == Ability.SCHOOLING
            and self._name == "Wishiwashi-school"
            and self.hp < self.starting_hp / 4
        ):
            if self.form("Wishiwashi"):
                msg += f"{self.name}'s school is gone!\n"
        if (
            self.ability() == Ability.SCHOOLING
            and self._name == "Wishiwashi"
            and self.hp >= self.starting_hp / 4
            and self.level >= 20
        ):
        ...
        if (
            self.ability() == Ability.POWER_CONSTRUCT
            and self._name in ("Zygarde", "Zygarde-10")
            and self.hp < self.starting_hp / 2
            and self.hp > 0
        ):
            if self.form("Zygarde-complete"):
                ...
                new_hp = round(
                    (
                        (
                            (
                                2 * self.base_stats["Zygarde-complete"][0]
                                + self.hpiv
                                + (self.hpev / 4)
                            )
                            * self.level
        ...
        if self.ability() == Ability.HUNGER_SWITCH:
            if self._name == "Morpeko":
                ...
                self.form("Morpeko")
        if (
            self.ability() == Ability.FLOWER_GIFT
            and self._name == "Cherrim"
            and battle.weather.get() in ("sun", "h-sun")
        ):
            self.form("Cherrim-sunshine")
        if (
            self.ability() == Ability.FLOWER_GIFT
            and self._name == "Cherrim-sunshine"
        ...
        # Bad Dreams
        if (
            otherpoke is not None
            and otherpoke.ability() == Ability.BAD_DREAMS
            and self.nv.sleep()
        ):
            msg += self.damage(
                self.starting_hp // 8, battle, source=f"{otherpoke.name}'s bad dreams"
            )
        # Leech seed
        ...
        if self.ability() == Ability.OVERCOAT:
            ...
            if (
                ...
                and self.ability()
                not in (Ability.SAND_RUSH, Ability.SAND_VEIL, Ability.SAND_FORCE)
            ):
                msg += self.damage(
                    self.starting_hp // 16, battle, source="the sandstorm"
                )
        elif battle.weather.get() == "hail":
            if ElementType.ICE not in self.type_ids and self.ability() not in (
                Ability.SNOW_CLOAK,
                Ability.ICE_BODY,
            ):
        ...
        if self.bind.next_turn():
            ...
            if otherpoke.held_item == "binding-band":
                msg += self.damage(
                    self.starting_hp // 6, battle, source=f"{otherpoke.name}'s bind"
                )
            else:
                msg += self.damage(
                    self.starting_hp // 8, battle, source=f"{otherpoke.name}'s bind"
                )
        # Ingrain
        if self.ingrain:
        ...
        if self.octolock and otherpoke is not None:
            msg += self.append_defense(
                StatChange(stage_delta=-1),
                attacker=self,
                source=f"{otherpoke.name}'s octolock",
            )
            msg += self.append_spdef(
                StatChange(stage_delta=-1),
                attacker=self,
        ...
        if (
            ...
            and self.grounded(battle)
            and not self.heal_block.active()
        ):
        ...
        if self.splinters.next_turn():
            ...
            msg += self.damage(
                self.starting_hp // 8, battle, source=f"{otherpoke.name}'s splinters"
            )
    ...
    def faint(self, battle, *, move=None, attacker=None, source: str = ""):
        ...
        if (
            ...
            and self.destiny_bond
            and self.owner.has_alive_pokemon()
        ):
            msg += attacker.faint(battle, source=f"{self.name}'s destiny bond")
        if (
            move is not None
            and attacker is not None
            and attacker._name == "Greninja"
            and attacker.ability() == Ability.BATTLE_BOND
        ):
        ...
        if attacker is not None and attacker.ability() in (
            Ability.CHILLING_NEIGH,
            Ability.AS_ONE_ICE,
        ):
            msg += attacker.append_attack(
                StatChange(stage_delta=1),
                attacker=attacker,
                source="its chilling neigh",
            )
        if attacker is not None and attacker.ability() in (
            Ability.GRIM_NEIGH,
            ...
            msg += attacker.append_spatk(
                StatChange(stage_delta=1), attacker=attacker, source="its grim neigh"
            )
        for poke in (battle.trainer1.current_pokemon, battle.trainer2.current_pokemon):
            if (
                poke is not None
                and poke is not self
                and poke.ability() == Ability.SOUL_HEART
            ):
                msg += poke.append_spatk(
                    StatChange(stage_delta=1), attacker=poke, source="its soul heart"
                )
    ...
    def _damage(
        ...
        *,
        ...
        # Magic guard
        if (
            self.ability(attacker=attacker, move=move) == Ability.MAGIC_GUARD
            and move is None
        ...
        if (
            ...
            and move.is_affected_by_substitute()
            and not move.is_sound_based()
        ...
        if move is not None:
            if (
                self.ability(attacker=attacker, move=move) == Ability.DISGUISE
                and self._name == "Mimikyu"
            ):
                if self.form("Mimikyu-busted"):
                    msg += f"{self.name}'s disguise was busted!\n"
                    msg += self.damage(
                        self.starting_hp // 8, battle, source="losing its disguise"
                    )
                    return msg, 0
            if (
                self.ability(attacker=attacker, move=move) == Ability.ICE_FACE
                and self._name == "Eiscue"
        ...
        if damage >= self.hp and move is not None:
            if self.endure:
                ...
                self.hp == self.starting_hp
                and self.ability(attacker=attacker, move=move) == Ability.STURDY
            ):
        ...
        dropped_below_quarter = (
            dropped_below_quarter and self.hp <= self.starting_hp / 4
        )
        if source:
            source = f" from {source}"
        ...
        if drain_heal_ratio is not None and attacker is not None:
            ...
            if self.ability() == Ability.LIQUID_OOZE:
                msg += attacker.damage(
                    heal, battle, source=f"{self.name}'s liquid ooze"
                )
            else:
        ...
        if self.hp == 0:
            ...
            if (
                ...
                msg += attacker.damage(
                    attacker.starting_hp // 4, battle, source=f"{self.name}'s aftermath"
                )
            if attacker is not None and attacker.ability() == Ability.MOXIE:
                msg += attacker.append_attack(
                    StatChange(stage_delta=1), attacker=attacker, source="its moxie"
                )
            ...
            if attacker is not None and self.ability() == Ability.INNARDS_OUT:
                msg += attacker.damage(
                    ...
                    battle,
                    attacker=self,
                    source=f"{self.name}'s innards out",
                )
            ...
            if (
                self.ability() == Ability.COLOR_CHANGE
                and move_type not in self.type_ids
            ):
            ...
            if self.ability() == Ability.ANGER_POINT and critical:
                msg += self.append_attack(
                    StatChange(stage_delta=6), attacker=self, source="its anger point"
                )
            if (
                self.ability() == Ability.WEAK_ARMOR
                and move.damage_class == DamageClass.PHYSICAL
                and attacker is not self
            ):
                msg += self.append_defense(
                    StatChange(stage_delta=-1), attacker=self, source="its weak armor"
                )
                msg += self.append_speed(
                    StatChange(stage_delta=2), attacker=self, source="its weak armor"
                )
            if self.ability() == Ability.JUSTIFIED and move_type == ElementType.DARK:
                msg += self.append_attack(
                    StatChange(stage_delta=1), attacker=self, source="justified"
                )
            if self.ability() == Ability.RATTLED and move_type in (
                ElementType.BUG,
                ElementType.DARK,
                ElementType.GHOST,
            ):
                msg += self.append_speed(
                    StatChange(stage_delta=1), attacker=self, source="its rattled"
                )
            if self.ability() == Ability.STAMINA:
                msg += self.append_defense(
                    StatChange(stage_delta=1), attacker=self, source="its stamina"
                )
            if (
                self.ability() == Ability.WATER_COMPACTION
                and move_type == ElementType.WATER
            ):
                msg += self.append_defense(
                    StatChange(stage_delta=2),
                    attacker=self,
                    source="its water compaction",
                )
            if self.ability() == Ability.BERSERK and dropped_below_half:
                msg += self.append_spatk(
                    StatChange(stage_delta=1), attacker=self, source="its berserk"
                )
            if self.ability() == Ability.STEAM_ENGINE and move_type in (
                ElementType.FIRE,
                ElementType.WATER,
            ):
                msg += self.append_speed(
                    StatChange(stage_delta=6), attacker=self, source="its steam engine"
                )
            ...
            if self.ability() == Ability.COTTON_DOWN and attacker is not None:
                msg += attacker.append_speed(
                    StatChange(stage_delta=-1),
                    attacker=self,
                    source=f"{self.name}'s cotton down",
                )
        ...
        if move is not None:
            ...
            if self.rage:
                msg += self.append_attack(
                    StatChange(stage_delta=1), attacker=self, source="its rage"
                )
            if attacker is not None:
                ...
                if (
                    attacker.ability() == Ability.MAGICIAN
                    and not attacker.held_item.has_item()
                    and self.held_item.can_remove()
                ):
                ...
                if attacker.held_item == "shell-bell":
                    ...
                    if (
                        attacker.ability() != Ability.SHEER_FORCE
                        or move.effect_chance is None
                    ):
        ...
        if (
            ...
            and self._name in ("Cramorant-gulping", "Cramorant-gorging")
            and self.owner.has_alive_pokemon()
        ):
            prey = "pikachu" if self._name == "Cramorant-gorging" else "arrokuda"
            if self.form("Cramorant"):
                msg += attacker.damage(
                    attacker.starting_hp // 4,
                    battle,
                    source=f"{self.name} spitting out its {prey}",
                )
                if prey == "arrokuda":
        ...
        if move is not None and attacker is not None and move.makes_contact(attacker):
            ...
            if attacker.held_item != "protective-pads":
                if self.beak_blast:
                    msg += attacker.nv.apply_status(
                        ...
                        battle,
                        attacker=self,
                        source=f"{self.name}'s charging beak blast",
                    )
                ...
                if self.ability() == Ability.POISON_POINT:
                    if random.randint(1, 100) <= 30:
                        msg += attacker.nv.apply_status(
                            ...
                            battle,
                            attacker=self,
                            source=f"{self.name}'s poison point",
                        )
                ...
                if (
                    self.ability() == Ability.ROUGH_SKIN
                    and self.owner.has_alive_pokemon()
                ):
                    msg += attacker.damage(
                        attacker.starting_hp // 8,
                        battle,
                        source=f"{self.name}'s rough skin",
                    )
                if (
                    self.ability() == Ability.IRON_BARBS
                    and self.owner.has_alive_pokemon()
                ):
                    msg += attacker.damage(
                        attacker.starting_hp // 8,
                        battle,
                        source=f"{self.name}'s iron barbs",
                    )
                ...
                if (
                    self.ability() == Ability.CUTE_CHARM
                    and random.randint(1, 100) <= 30
                ):
                    msg += attacker.infatuate(self, source=f"{self.name}'s cute charm")
                if (
                    self.ability() == Ability.MUMMY
                    and attacker.ability() != Ability.MUMMY
                    and attacker.ability_changeable()
                ):
                ...
                if self.ability() == Ability.GOOEY:
                    msg += attacker.append_speed(
                        StatChange(stage_delta=-1),
                        attacker=self,
                        source=f"touching {self.name}'s gooey body",
                    )
                if self.ability() == Ability.TANGLING_HAIR:
                    msg += attacker.append_speed(
                        StatChange(stage_delta=-1),
                        attacker=self,
                        source=f"touching {self.name}'s tangled hair",
                    )
                if self.held_item == "rocky-helmet" and self.owner.has_alive_pokemon():
                    msg += attacker.damage(
                        attacker.starting_hp // 6,
                        battle,
                        source=f"{self.name}'s rocky helmet",
                    )
            ...
            if attacker.ability() == Ability.POISON_TOUCH:
                if random.randint(1, 100) <= 30:
                    msg += self.nv.apply_status(
                        ...
                        attacker=attacker,
                        move=move,
                        source=f"{attacker.name}'s poison touch",
                    )
            # Affects BOTH
            if attacker.held_item != "protective-pads":
                if (
                    self.ability() == Ability.PERISH_BODY
                    and not attacker.perish_song.active()
                ):
            ...
            if (
                self.ability() == Ability.WANDERING_SPIRIT
                and attacker.ability_changeable()
                ...
                self.ability_id, attacker.ability_id = (
                    attacker.ability_id,
                    self.ability_id,
                )
                ability_name = Ability(self.ability_id).pretty_name
    ...
    def flinch(self, *, attacker=None, move=None, source=""):
        ...
        if self.ability() == Ability.STEADFAST:
            msg += self.append_speed(
                StatChange(stage_delta=1), attacker=self, source="its steadfast"
            )
        return msg
    ...
    def calculate_stat(self, stat, stat_changes, *, crop=False):
        """Calculates a stat based on that stat's stage changes."""
        stage = self.calculate_stat_stage(stat_changes)
        if crop == "bottom":
            stage = max(stage, 0)
        elif crop == "top":
            stage = min(stage, 0)
        ...
        return stage_multiplier[stage] * stat

    @staticmethod
    def calculate_stat_stage(stat_changes):
        """Calculates the current stat stage for a particluar stat."""
        stage = 0
        for stat in stat_changes:
            stage += stat.stage_delta
        return min(6, max(-6, stage))
    ...
    def get_raw_attack(self, *, check_power_trick=True, check_power_shift=True):
        ...
        if self.power_trick and check_power_trick:
            return self.get_raw_defense(
                check_power_trick=False, check_power_shift=check_power_shift
            )
        if self.power_shift and check_power_shift:
            return self.get_raw_defense(
                check_power_trick=check_power_trick, check_power_shift=False
            )
        stat = self.calculate_raw_stat(
            self.attack, self.atkiv, self.atkev, self.nature_stat_deltas["Attack"]
        )
    ...
    def get_raw_defense(self, *, check_power_trick=True, check_power_shift=True):
        ...
        stat = self.calculate_raw_stat(
            self.defense, self.defiv, self.defev, self.nature_stat_deltas["Defense"]
        )
    ...
    def get_raw_spatk(self, *, check_power_shift=True):
        ...
        stat = self.calculate_raw_stat(
            ...
            self.spatkiv,
            self.spatkev,
            self.nature_stat_deltas["Special attack"],
        )
    ...
    def get_raw_spdef(self, *, check_power_shift=True):
        ...
        stat = self.calculate_raw_stat(
            ...
            self.spdefiv,
            self.spdefev,
            self.nature_stat_deltas["Special defense"],
        )
    ...
    def get_raw_speed(self):
        """Returns the raw attack of this poke, taking into account IVs EVs and natures and forms."""
        return self.calculate_raw_stat(
            self.speed, self.speediv, self.speedev, self.nature_stat_deltas["Speed"]
        )
    ...
    def get_attack(self, battle, *, critical=False, ignore_stages=False):
        ...
        if not ignore_stages:
            attack = self.calculate_stat(
                attack, self.atk_changes, crop="bottom" if critical else False
            )
        ...
        if self.ability() == Ability.GORILLA_TACTICS:
            attack *= 1.5
        if self.ability() == Ability.FLOWER_GIFT and battle.weather.get() in (
            "sun",
            "h-sun",
        ):
            attack *= 1.5
        ...
        if self.held_item == "light-ball" and self._name == "Pikachu":
            attack *= 2
        if self.held_item == "thick-club" and self._name in (
            "Cubone",
            "Marowak",
            "Marowak-alola",
        ):
    ...
    def get_defense(
        self, battle, *, critical=False, ignore_stages=False, attacker=None, move=None
    ):
        ...
        if not ignore_stages:
            defense = self.calculate_stat(
                defense, self.def_changes, crop="top" if critical else False
            )
        if (
            self.ability(attacker=attacker, move=move) == Ability.MARVEL_SCALE
            and self.nv.current
        ):
            defense *= 1.5
        if self.ability(attacker=attacker, move=move) == Ability.FUR_COAT:
            defense *= 2
        if (
            self.ability(attacker=attacker, move=move) == Ability.GRASS_PELT
    ...
    def get_spatk(self, battle, *, critical=False, ignore_stages=False):
        ...
        if not ignore_stages:
            spatk = self.calculate_stat(
                spatk, self.spatk_changes, crop="bottom" if critical else False
            )
        if self.ability() == Ability.DEFEATIST and self.hp <= self.starting_hp / 2:
            spatk *= 0.5
        if self.ability() == Ability.SOLAR_POWER and battle.weather.get() in (
    ...
    def get_spdef(
        ...
        if not ignore_stages:
            spdef = self.calculate_stat(
                spdef, self.spdef_changes, crop="top" if critical else False
            )
        if battle.weather.get() == "sandstorm" and ElementType.ROCK in self.type_ids:
            spdef *= 1.5
        if self.ability(
            attacker=attacker, move=move
        ) == Ability.FLOWER_GIFT and battle.weather.get() in ("sun", "h-sun"):
            spdef *= 1.5
    ...
    def get_speed(self, battle):
        ...
        speed = self.calculate_stat(self.get_raw_speed(), self.speed_changes)
        ...
        if self.ability() == Ability.SAND_RUSH and battle.weather.get() == "sandstorm":
            speed *= 2
        if self.ability() == Ability.SWIFT_SWIM and battle.weather.get() in (
            "rain",
            "h-rain",
        ):
            speed *= 2
        if self.ability() == Ability.CHLOROPHYLL and battle.weather.get() in (
            "sun",
            "h-sun",
        ):
            speed *= 2
        if self.ability() == Ability.SLOW_START and self.active_turns < 5:
            speed *= 0.5
        if (
            self.ability() == Ability.UNBURDEN
            and not self.held_item.has_item()
            and self.held_item.ever_had_item
        ):
            speed *= 2
    ...
    def get_accuracy(self, battle):
        ...
        return self.calculate_stat_stage(self.accuracy_changes)
    ...
    def append_attack(
        self, stat_change: StatChange, *, attacker=None, move=None, source: str = ""
    ):
        """Helper method to call append_stat for attack."""
        return self.append_stat(
            stat_change, attacker, move, "attack", self.atk_changes, source
        )
    ...
    def append_defense(
        ...
        return self.append_stat(
            stat_change, attacker, move, "defense", self.def_changes, source
        )
    ...
    def append_spatk(
        ...
        return self.append_stat(
            stat_change, attacker, move, "special attack", self.spatk_changes, source
        )
    ...
    def append_spdef(
        ...
        return self.append_stat(
            stat_change, attacker, move, "special defense", self.spdef_changes, source
        )
    ...
    def append_speed(
        ...
        return self.append_stat(
            stat_change, attacker, move, "speed", self.speed_changes, source
        )
    ...
    def append_accuracy(
        ...
        return self.append_stat(
            stat_change, attacker, move, "accuracy", self.accuracy_changes, source
        )
    ...
    def append_evasion(
        ...
        return self.append_stat(
            stat_change, attacker, move, "evasion", self.evasion_changes, source
        )
    ...
    def append_stat(
        ...
        stat_change.stage_delta = delta
        if delta < 0 and attacker is not self:
            if self.ability(attacker=attacker, move=move) in (
                Ability.CLEAR_BODY,
                Ability.WHITE_SMOKE,
            ...
            if (
                self.ability(attacker=attacker, move=move) == Ability.HYPER_CUTTER
                and stat == "attack"
            ):
                return (
                    f"{self.name}'s claws stayed sharp because of its hyper cutter!\n"
                )
            if (
                self.ability(attacker=attacker, move=move) == Ability.KEEN_EYE
                and stat == "accuracy"
            ):
            ...
            if (
                ...
                return (
                    f"{self.name}'s defense stayed strong because of its big pecks!\n"
                )
            if self.owner.mist.active() and (
            ...
            if self.ability(attacker=attacker, move=move) == Ability.DEFIANT:
                msg += self.append_attack(
                    StatChange(stage_delta=2), attacker=self, source="its defiance"
                )
            if self.ability(attacker=attacker, move=move) == Ability.COMPETITIVE:
                msg += self.append_spatk(
                    StatChange(stage_delta=2),
                    attacker=self,
                    source="its competitiveness",
                )
            if (
                self.ability(attacker=attacker, move=move) == Ability.FLOWER_VEIL
                and ElementType.GRASS in self.type_ids
            ):
                return ""
            if (
                self.ability(attacker=attacker, move=move) == Ability.MIRROR_ARMOR
                and attacker is not None
            ):
    ...
    def transform(self, otherpoke):
        ...
        self.atk_changes = [change.copy() for change in otherpoke.atk_changes]
        self.def_changes = [change.copy() for change in otherpoke.def_changes]
        self.spatk_changes = [change.copy() for change in otherpoke.spatk_changes]
        self.spdef_changes = [change.copy() for change in otherpoke.spdef_changes]
        self.speed_changes = [change.copy() for change in otherpoke.speed_changes]
        self.accuracy_changes = [change.copy() for change in otherpoke.accuracy_changes]
        self.evasion_changes = [change.copy() for change in otherpoke.evasion_changes]
    ...
    def effectiveness(
        self, attacker_type: ElementType, battle, *, attacker=None, move=None
    ):
        ...
        for defender_type in self.type_ids:
            ...
            if (
                move is not None
                and move.effect == 380
                and defender_type == ElementType.WATER
            ):
                effectiveness *= 2
                continue
            if (
                move is not None
                and move.effect == 373
                and defender_type == ElementType.FLYING
                and not self.grounded(battle, attacker=attacker, move=move)
            ):
            ...
            if (
                self.foresight
                and attacker_type in (ElementType.FIGHTING, ElementType.NORMAL)
                and defender_type == ElementType.GHOST
            ):
                continue
            if (
                self.miracle_eye
                and attacker_type == ElementType.PSYCHIC
                and defender_type == ElementType.DARK
            ):
                continue
            if (
                ...
                continue
            if (
                attacker_type == ElementType.GROUND
                and defender_type == ElementType.FLYING
                and self.grounded(battle, attacker=attacker, move=move)
            ):
                continue
            e = battle.type_effectiveness[(attacker_type, defender_type)] / 100
            if (
                defender_type == ElementType.FLYING
                and e > 1
                and move is not None
                and battle.weather.get() == "h-wind"
            ):
                e = 1
    ...
    async def create(cls, ctx, raw_data: dict):
        ...
        stats = (
            await find_one(
                ctx, "pokemon_stats", {"pokemon_id": form_info["pokemon_id"]}
            )
        )["stats"]
        pokemonHp = stats[0]
        ...
        if mega_form is not None:
            mega_form_info = await find_one(
                ctx, "forms", {"identifier": mega_form.lower()}
            )
            if mega_form_info is not None:
                mega_ability = await find_one(
                    ctx, "poke_abilities", {"pokemon_id": mega_form_info["pokemon_id"]}
                )
                ...
                mega_types = await find_one(
                    ctx, "ptypes", {"id": mega_form_info["pokemon_id"]}
                )
        ...
        for f_name in extra_forms:
            ...
            f_stats = (
                await find_one(
                    ctx, "pokemon_stats", {"pokemon_id": f_info["pokemon_id"]}
                )
            )["stats"]
        ...
        ab_ids = []
        for record in await find(
            ctx, "poke_abilities", {"pokemon_id": form_info["pokemon_id"]}
        ):
        ...
        can_still_evolve = bool(
            await find_one(ctx, "pfile", {"evolves_from_species_id": pid})
        )

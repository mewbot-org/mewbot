...
class DuelPokemon:
    ...
    def get_attack(self, battle, *, critical=False, ignore_stages=False):
        ...
        if (
            ...
            and (
                battle.weather.get() in ("sun", "h-sun")
                or self.held_item == "booster-energy"
            )
        ) or (
            self.ability() == Ability.QUARK_DRIVE
            and (
                battle.terrain.item == "electric" or self.held_item == "booster-energy"
            )
        ):
            if all(
    ...
    async def create(cls, ctx, raw_data: dict):
        ...
        for move in moves:
            ...
            move = await find_one(ctx, "dduel_moves", {"identifier": move})
            if move is None:
                move = await find_one(ctx, "dduel_moves", {"identifier": "tackle"})
            else:

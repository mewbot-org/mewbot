...
class DuelPokemon:
    ...
    def send_out_ability(self, otherpoke, battle):
        ...
        if self.ability() == Ability.INTIMIDATE and otherpoke is not None:
            if otherpoke.ability() == Ability.OBLIVIOUS:
                ...
                msg += (
                    f"{otherpoke.name}'s guard dog keeps it from being intimidated!\n"
                )
                msg += otherpoke.append_attack(
                    1, attacker=otherpoke, source="its guard dog"
                )
            else:
                msg += otherpoke.append_attack(
                    -1, attacker=self, source=f"{self.name}'s Intimidate"
                )
                if otherpoke.held_item == "adrenaline-orb":
                    msg += otherpoke.append_speed(
                        1, attacker=otherpoke, source="its adrenaline orb"
                    )
                if otherpoke.ability() == Ability.RATTLED:
                    msg += otherpoke.append_speed(
                        1, attacker=otherpoke, source="its rattled"
                    )
    ...
    def next_turn(self, otherpoke, battle):
        ...
        if self.yawn.next_turn():
            msg += self.nv.apply_status(
                "sleep", battle, attacker=self, source="drowsiness"
            )
        ...
        if self.encore.active() and self.encore.item.pp == 0:
            ...
            msg += f"{self.name}'s encore is over!\n"
        if (
            self.cud_chew.next_turn()
            and self.held_item.last_used is not None
            and self.held_item.last_used.name.endswith("-berry")
        ):
        ...
        if self.octolock and otherpoke is not None:
            msg += self.append_defense(
                -1, attacker=self, source=f"{otherpoke.name}'s octolock"
            )
    ...
    def faint(self, battle, *, move=None, attacker=None, source: str = ""):
        ...
        if attacker is not None and attacker.ability() in (
            ...
            msg += attacker.append_attack(
                1, attacker=attacker, source="its chilling neigh"
            )
        if attacker is not None and attacker.ability() in (
            Ability.GRIM_NEIGH,
        ...
        for poke in (battle.trainer1.current_pokemon, battle.trainer2.current_pokemon):
            if (
                ...
                msg += poke.append_spatk(1, attacker=poke, source="its soul heart")
        ...
        *,
        ...
        if self.hp == 0:
            ...
            if (
                ...
                msg += self.append_defense(
                    2, attacker=self, source="its water compaction"
                )
            ...
            if (
                self.ability() == Ability.THERMAL_EXCHANGE
                and move_type == ElementType.FIRE
            ):
                msg += self.append_attack(
                    1, attacker=self, source="its thermal exchange"
                )
            ...
            if self.ability() == Ability.COTTON_DOWN and attacker is not None:
                msg += attacker.append_speed(
                    -1, attacker=self, source=f"{self.name}'s cotton down"
                )
            ...
            if (
                self.ability() == Ability.TOXIC_DEBRIS
                and move.damage_class == DamageClass.PHYSICAL
            ):
                if (
                    attacker is not None
                    and attacker is not self
                    and attacker.owner.toxic_spikes < 2
                ):
            ...
            if self.form("Cramorant"):
                ...
                if prey == "arrokuda":
                    msg += attacker.append_defense(
                        -1, attacker=self, source=f"{self.name} spitting out its {prey}"
                    )
                elif prey == "pikachu":
            ...
            if attacker.held_item != "protective-pads":
                ...
                if self.ability() == Ability.EFFECT_SPORE:
                    if (
                        ...
                        msg += attacker.nv.apply_status(
                            status, battle, attacker=attacker
                        )
                if (
                ...
                if (
                    self.ability() == Ability.LINGERING_AROMA
                    and attacker.ability() != Ability.LINGERING_AROMA
                    and attacker.ability_changeable()
                ):
                ...
                if self.ability() == Ability.GOOEY:
                    msg += attacker.append_speed(
                        -1, attacker=self, source=f"touching {self.name}'s gooey body"
                    )
                if self.ability() == Ability.TANGLING_HAIR:
                    msg += attacker.append_speed(
                        -1, attacker=self, source=f"touching {self.name}'s tangled hair"
                    )
    ...
    def get_attack(self, battle, *, critical=False, ignore_stages=False):
        ...
        if not ignore_stages:
            attack = self.calculate_stat(
                attack, self.attack_stage, crop="bottom" if critical else False
            )
        ...
        if self.ability() == Ability.FLOWER_GIFT and battle.weather.get() in (
            ...
            attack *= 1.5
        if self.ability() == Ability.ORICHALCUM_PULSE and battle.weather.get() in (
            "sun",
            "h-sun",
        ):
            attack *= 4 / 3
        ...
        for poke in (battle.trainer1.current_pokemon, battle.trainer2.current_pokemon):
            if (
                poke is not None
                and poke is not self
                and poke.ability() == Ability.TABLETS_OF_RUIN
            ):
                attack *= 0.75
        if all(
            self.get_raw_attack() >= x
            for x in (
                self.get_raw_defense(),
                self.get_raw_spatk(),
                self.get_raw_spdef(),
                self.get_raw_speed(),
            )
            ...
            if (
                ...
                and (battle.weather.get() in ("sun", "h-sun") or self.booster_energy)
            ) or (
                self.ability() == Ability.QUARK_DRIVE
                and (battle.terrain.item == "electric" or self.booster_energy)
            ):
                attack *= 1.3
            elif (
                self.ability() in (Ability.PROTOSYNTHESIS, Ability.QUARK_DRIVE)
                and self.held_item == "booster-energy"
            ):
                self.held_item.use()
    ...
    def get_defense(
        ...
        if not ignore_stages:
            defense = self.calculate_stat(
                defense, self.defense_stage, crop="top" if critical else False
            )
        if (
        ...
        for poke in (battle.trainer1.current_pokemon, battle.trainer2.current_pokemon):
            if (
                poke is not None
                and poke is not self
                and poke.ability() == Ability.SWORD_OF_RUIN
            ):
                defense *= 0.75
        if self.get_raw_defense() > self.get_raw_attack() and all(
            self.get_raw_defense() >= x
            for x in (self.get_raw_spatk(), self.get_raw_spdef(), self.get_raw_speed())
        ):
            if (
                self.ability() == Ability.PROTOSYNTHESIS
                and (battle.weather.get() in ("sun", "h-sun") or self.booster_energy)
    ...
    def get_spatk(self, battle, *, critical=False, ignore_stages=False):
        ...
        if not ignore_stages:
            spatk = self.calculate_stat(
                spatk, self.spatk_stage, crop="bottom" if critical else False
            )
        if self.ability() == Ability.DEFEATIST and self.hp <= self.starting_hp / 2:
            spatk *= 0.5
        if self.ability() == Ability.SOLAR_POWER and battle.weather.get() in (
        ...
        for poke in (battle.trainer1.current_pokemon, battle.trainer2.current_pokemon):
            if (
                poke is not None
                and poke is not self
                and poke.ability() == Ability.VESSEL_OF_RUIN
            ):
                spatk *= 0.75
        if all(
            self.get_raw_spatk() >= x
            for x in (self.get_raw_spdef(), self.get_raw_speed())
        ) and all(
            self.get_raw_spatk() > x
            for x in (self.get_raw_attack(), self.get_raw_defense())
        ):
            if (
                self.ability() == Ability.PROTOSYNTHESIS
                and (battle.weather.get() in ("sun", "h-sun") or self.booster_energy)
    ...
    def get_spdef(
        ...
        if not ignore_stages:
            spdef = self.calculate_stat(
                spdef, self.spdef_stage, crop="top" if critical else False
            )
        ...
        for poke in (battle.trainer1.current_pokemon, battle.trainer2.current_pokemon):
            if (
                poke is not None
                and poke is not self
                and poke.ability() == Ability.BEADS_OF_RUIN
            ):
                spdef *= 0.75
        if self.get_raw_spdef() >= self.get_raw_speed() and all(
            self.get_raw_spdef() > x
            for x in (
                self.get_raw_attack(),
                self.get_raw_defense(),
                self.get_raw_spatk(),
    ...
    def append_attack(
        ...
        *,
        ...
        move=None,
        source: str = "",
        check_looping: bool = True,
    ):
        """Helper method to call append_stat for attack."""
        return self.append_stat(
            stage_change, attacker, move, "attack", source, check_looping
        )
    ...
    def append_defense(
        ...
        *,
        ...
        return self.append_stat(
            stage_change, attacker, move, "defense", source, check_looping
        )
    ...
    def append_spatk(
        ...
        *,
        ...
        return self.append_stat(
            stage_change, attacker, move, "special attack", source, check_looping
        )
    ...
    def append_spdef(
        ...
        *,
        ...
        return self.append_stat(
            stage_change, attacker, move, "special defense", source, check_looping
        )
    ...
    def append_speed(
        ...
        *,
        ...
        return self.append_stat(
            stage_change, attacker, move, "speed", source, check_looping
        )
    ...
    def append_accuracy(
        ...
        *,
        ...
        return self.append_stat(
            stage_change, attacker, move, "accuracy", source, check_looping
        )
    ...
    def append_evasion(
        ...
        *,
        ...
        return self.append_stat(
            stage_change, attacker, move, "evasion", source, check_looping
        )
    ...
    def append_stat(
        ...
        if (
            ...
            and attacker is not self
            and attacker is not None
            and (move is None or move.is_affected_by_substitute())
        ):
            return ""
        if source:
            source = f" from {source}"
        ...
        if delta < 0 and attacker is not self:
            ...
            if (
                ...
                msg += attacker.append_stat(
                    delta, self, None, stat, "", check_looping=False
                )
                return msg
        ...
        if delta < 0:
            if attacker is not self:
                ...
                if self.ability(attacker=attacker, move=move) == Ability.COMPETITIVE:
                    msg += self.append_spatk(
                        2, attacker=self, source="its competitiveness"
                    )
            ...
            for poke in (
                ...
                if (
                    poke is not None
                    and poke is not self
                    and poke.ability() == Ability.OPPORTUNIST
                    and check_looping
                ):
                    msg += f"{poke.name} seizes the opportunity to boost its stat with its opportunist!\n"
                    msg += poke.append_stat(
                        delta, poke, None, stat, "", check_looping=False
                    )
        return msg
    ...
    def ability(self, *, attacker=None, move=None):
        ...
        if attacker.ability_id in (
            ...
            return None
        if (
            attacker.ability_id == Ability.MYCELIUM_MIGHT
            and move.damage_class == DamageClass.STATUS
        ):
            return None
        return self.ability_id

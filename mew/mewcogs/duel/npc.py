...
async def move_generator(self, npc_pokemon: str, user_pokemon: str):
    ...
    form_info = await self.bot.db[1].forms.find_one(
        {"identifier": user_pokemon.lower()}
    )
    type_ids = (await self.bot.db[1].ptypes.find_one({"id": form_info["pokemon_id"]}))[
        "types"
    ]
    type_effectiveness = {}
    ...
    super_raw = (
        await self.bot.db[1]
        .moves.find({"type_id": {"$in": super_types}, "damage_class_id": {"$ne": 1}})
        .to_list(None)
    )
    super_moves = [x["identifier"] for x in super_raw]
    normal_raw = (
        await self.bot.db[1]
        .moves.find({"type_id": {"$in": normal_types}, "damage_class_id": {"$ne": 1}})
        .to_list(None)
    )
...
async def generate_pokemon(
    self, npc_pokemon, user_pokemon, user_pokemon_level, npc_type=None
):

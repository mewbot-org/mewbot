...
class Move:
    ...
    def setup(self, attacker, defender, battle):
        ...
        if self.effect == 129 and (
            ...
            or defender.owner.selected_action.effect in (128, 154, 229, 347, 493)
        ):
    ...
    def use(
        ...
        *,
        ...
        if (
            self.effect in (5, 126, 168, 254, 336, 398, 458, 500)
            and attacker.nv.freeze()
        ):
            attacker.nv.reset()
        ...
        # Turn 2 hit moves
            ...
            self.effect
            in (40, 76, 146, 152, 156, 256, 257, 264, 273, 332, 333, 366, 451, 502)
        ...
        if not defender.substitute:
            if (
                current_type == ElementType.WATER
                and defender.held_item == "absorb-bulb"
            ):
                msg += defender.append_spatk(
                    1, attacker=defender, move=self, source="its absorb bulb"
                )
                defender.held_item.use()
            if (
                current_type == ElementType.ELECTRIC
                and defender.held_item == "cell-battery"
            ):
                msg += defender.append_attack(
                    1, attacker=defender, move=self, source="its cell battery"
                )
                defender.held_item.use()
            if (
                current_type == ElementType.WATER
                and defender.held_item == "luminous-moss"
            ):
                msg += defender.append_spdef(
                    1, attacker=defender, move=self, source="its luminous moss"
                )
                defender.held_item.use()
            if current_type == ElementType.ICE and defender.held_item == "snowball":
                msg += defender.append_attack(
                    1, attacker=defender, move=self, source="its snowball"
                )
                defender.held_item.use()
        ...
        if self.effect == 162:
            msg += attacker.append_defense(
                -attacker.stockpile, attacker=attacker, move=self
            )
        ...
        # A red-card forces the attacker to swap to a random poke, even if they used a switch out move
            ...
            if not swaps:
                ...
                msg += (
                    f"{defender.name} held up its red card against {attacker.name}!\n"
                )
                defender.held_item.use()
        ...
        if self.effect == 251:
            ...
            attacker.spatk_stage, defender.spatk_stage = (
                defender.spatk_stage,
                attacker.spatk_stage,
            )
            attacker.spdef_stage, defender.spdef_stage = (
                defender.spdef_stage,
                attacker.spdef_stage,
            )
            attacker.speed_stage, defender.speed_stage = (
                defender.speed_stage,
                attacker.speed_stage,
            )
            attacker.accuracy_stage, defender.accuracy_stage = (
                defender.accuracy_stage,
                attacker.accuracy_stage,
            )
            attacker.evasion_stage, defender.evasion_stage = (
                defender.evasion_stage,
                attacker.evasion_stage,
            )
        ...
        if self.effect == 226:
            ...
            if attacker.ability() == Ability.WIND_RIDER:
                msg += attacker.append_attack(
                    1, attacker=attacker, source="its wind rider"
                )
        ...
        if self.is_sound_based() and attacker.held_item == "throat-spray":
            msg += attacker.append_spatk(
                1, attacker=attacker, source="it's throat spray"
            )
            attacker.held_item.use()
    ...
    def attack(self, attacker, defender, battle):
        ...
        if self.effect == 361 and attacker._name == "Greninja-ash":
            ...
            if attacker.ability() == Ability.SKILL_LINK:
                ...
                attacker.held_item == "loaded-dice"
                and max_hits >= 4
                and (min_hits < 4 or self.effect == 484)
            ):
                min_hits = 4
        ...
        for hit in range(hits):
            ...
            if (
                ...
                damage *= 0.5
            if (
                defender.ability(attacker=attacker, move=self) == Ability.PURIFYING_SALT
                and current_type == ElementType.GHOST
            ):
                damage *= 0.5
            if (
                Ability.DARK_AURA
        ...
        if (
            ...
            msg += defender.append_attack(
                2, attacker=defender, move=self, source="its weakness policy"
            )
            msg += defender.append_spatk(
                2, attacker=defender, move=self, source="its weakness policy"
            )
    ...
    def get_power(self, attacker, defender, battle):
        ...
        if (
            ...
            power *= 1.5
        if (
            attacker.ability() == Ability.ROCKY_PAYLOAD
            and current_type == ElementType.ROCK
        ):
            power *= 1.5
    ...
    def check_executable(self, attacker, defender, battle):
        ...
        if self.effect == 29 and not defender.owner.valid_swaps(
            ...
            return False
        if self.effect in (128, 154, 493) and not attacker.owner.valid_swaps(
        ...
        if self.effect == 145 and attacker.last_move_damage[1] != DamageClass.SPECIAL:
            return False
        if (
            self.effect in (90, 408)
            and attacker.last_move_damage[1] != DamageClass.PHYSICAL
        ):
            return False
        ...
        if self.effect == 341 and defender.owner.sticky_web:
            return False
        if (
            self.effect in (112, 117, 356, 362, 384, 454, 488, 499)
        ...
        if self.effect == 83 and self not in attacker.moves:
            return False
        if self.effect == 501 and (
            defender.has_moved
            or isinstance(defender.owner.selected_action, int)
            or defender.owner.selected_action.get_priority(defender, attacker, battle)
            <= 0
        ):
            return False
        if (
            defender.ability(attacker=attacker, move=self)
            in (Ability.QUEENLY_MAJESTY, Ability.DAZZLING, Ability.ARMOR_TAIL)
        ...
        if defender.king_shield and self.damage_class != DamageClass.STATUS:
            if self.makes_contact(attacker) and attacker.held_item != "protective-pads":
                msg += attacker.append_attack(
                    ...
                    attacker=defender,
                    move=self,
                    source=f"{defender.name}'s silk trap",
                )
            return False, msg
        ...
        if defender.burning_bulwark and self.damage_class != DamageClass.STATUS:
            if self.makes_contact(attacker) and attacker.held_item != "protective-pads":
                msg += attacker.nv.apply_status(
                    ...
                    battle,
                    attacker=defender,
                    source=f"{defender.name}'s burning bulwark",
                )
            return False, msg
    ...
    def check_hit(self, attacker, defender, battle):
        ...
        if self.effect in (153, 334, 357, 365, 396) and battle.weather.get() in (
            "rain",
            "h-rain",
        ):
            return True
    ...
    def check_effective(self, attacker, defender, battle):
        ...
        if (
            ...
            if (
                ...
                return False
        if (
            defender.ability(attacker=attacker, move=self) == Ability.GOOD_AS_GOLD
            and self.damage_class == DamageClass.STATUS
        ):
            return False

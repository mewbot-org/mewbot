...
async def generate_team_preview(battle):
    ...
    player1_pokemon_info = [(pokemon._name.replace(" ", "-"), pokemon.level) for pokemon in battle.trainer1.party]
    player1_data = {
        "name": battle.trainer1.name,
        "pokemon_info": [
            (
                "pixel_sprites/" + await get_file_name(
                    pokemon[0],
                    battle.ctx.bot,
                ), pokemon[1]
    ...
    player2_pokemon_info = [(pokemon._name.replace(" ", "-"), pokemon.level) for pokemon in battle.trainer2.party]
    player2_data = {
    ...
    await battle.ctx.send(embed=e, file=discord.File(image, "team_preview.png"), view=preview_view)
...
async def generate_main_battle_message(battle):
    ...
    try: #aiohttp 3.7 introduced a bug in dpy which causes this to error when rate limited. This catch just lets the bot continue when that happens.
        ...
        await battle.ctx.send(embed=e, file=discord.File(image, "battle.png"), view=battle_view)
    except RuntimeError:
        pass
    return battle_view

    ...
    def __init__(self):
        options = [
            discord.SelectOption(
                label="Breeder1", emoji="<:breeder1:1196868278056923328>"
            ),
            discord.SelectOption(
                label="Breeder2", emoji="<:breeder2:1196868276014297158>"
            ),
        ]
...
class Achievements(commands.Cog):
    ...
    async def on_egg_born(
        self, channel, user, achievement: str = None, gained_count: int = 1
    ):
        ...
        async with self.bot.db[0].acquire() as pconn:
            ...
            threshold, msg, level_up = threshold_check(
    ...
    async def achievements_breeding(self, ctx):
        ...
        if len(achievement_data) != 0:
            for i in range(0, 3):
                ...
                thresold, msg, level_up = threshold_check(
                    new_count=current_threshold, current_count=current_threshold
                )
        ...
        if category == "Breeding":
            async with ctx.bot.db[0].acquire() as pconn:
                ...
                if breed_success >= 1000:
                    ...
                    await ctx.send(
                        "You have successfully redeemed 10 Legend Chest <:legend_chest:1103389711424294942>!"
                    )
                    return
            ...
            if breed_success >= 250:
                if "breeder" in trainer_images:
                    await ctx.send(
                        "You've already claimed the 250 successful breeds reward!"
                    )
                    return

...
class DropdownSelect(discord.ui.Select):
    def __init__(self, bag_data):
        options = [
            discord.SelectOption(
                label="General",
                description="General Items, Crystals, Vitamins",
                emoji="<:general_items:1093297547805720576>",
            ),
            discord.SelectOption(
                label="Evostones",
                description="Type and Mega stones",
                emoji="<:powerup:672623832514822189>",
            ),
            discord.SelectOption(
                label="Trades",
                description="Items held during trades",
                emoji="<:general:672627507278905355>",
            ),
            discord.SelectOption(
                label="Plates and Memories",
                description="Items for Arceus and Silvally",
                emoji="<:crystalsbag:682684746911973392>",
            ),
            discord.SelectOption(
                label="Forms",
                description="Change Pokemon forms",
                emoji="<:tradeitems:672816581897748480>",
            ),
            discord.SelectOption(
                label="Battles",
                description="Used by Pokemon during duels",
                emoji="<:battlebag:674293206690562099>",
            ),
            discord.SelectOption(
                label="Berries",
                description="Berries gained through farming",
                emoji="<:bag_berry:741137148316614696>",
            ),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.view.choice == "Evostones":
            ...
            embed.set_footer(text=f"{interaction.user.name}'s bag")
...
class Bag(commands.Cog):
    ...
    async def view(self, ctx):
        ...
        await BagView(ctx, bag_data).wait()
    ...
    async def convert(self, ctx):
        ...
        async with ctx.bot.db[0].acquire() as conn:
            ...
            await conn.execute(
                "UPDATE users SET bag_convert = True WHERE u_id = $1", ctx.author.id
            )
                ...
                try:
                    ...
                    if "-rod" not in item:
                        await self.bot.commondb.add_bag_item(
                            ctx.author.id, item.replace("-", "_").lower(), dets[item]
                        )
                    ...
                    if item not in ("poison_barb", "sun_stone", "ultra_destiny_knot"):
                        msg += f"{item}\n"
        await ctx.send(
            f"Normal bag completed, starting on account bound items <@{ctx.author.id}>"
        )
        for item in bound_dets:
            if bound_dets[item] > 0:
                try:
                    ...
                    # Accounts for mistype in some user's bag
                    await self.bot.commondb.add_bag_item(
                        ctx.author.id, item, amount_gained, True
                    )
    ...
    async def old(self, ctx):
        ...
        embed = discord.Embed(title="Your Current Bag", color=0xFFB6C1)

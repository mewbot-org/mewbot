...
class AsyncIter:
    ...
    def __init__(
        self, iterable, delay = 0, steps: int = 1
    ) -> None:
        if steps < 1:
...
async def get_pokemon_image(name, bot, shiny=None, *, radiant=False, skin=None):
    ...
    return await get_spawn_url(await get_file_name(name, bot, shiny, radiant=radiant, skin=skin))
...
async def get_file_name(name, bot, shiny=False, *, radiant=False, skin=None):
    ...
    if not identifier:
        ...
        raise ValueError(f"Invalid name ({name}) passed to mew/utils/misc.py get_file_name.")
    ...
    if suffix and name.endswith(suffix):
        ...
        pokemon_identifier = await bot.db[1].forms.find_one(
...
async def get_battle_image(poke1, poke2, bot):
    ...
    base_url = await get_pokemon_image(poke1.name, bot, poke1.shiny, radiant=poke1.radiant)
    _base_url = await get_pokemon_image(poke2.name, bot, poke2.shiny, radiant=poke2.radiant)
...
class MenuView(discord.ui.View):
    ...
    def __init__(self, ctx: "commands.Context", pages: "List[Union[str, discord.Embed]]"):
...
class EnableCommandsView(discord.ui.View):
    ...
    async def reenable(self, button, interaction):
        ...
        if self.ctx.channel.id not in disabled:
            await interaction.response.send_message(content="Commands have already been re-enabled.")
            return
        ...
        await interaction.response.send_message(content=f"Successfully enabled commands in {self.ctx.channel}.")
